...
from config import TELEGRAM_TOKEN, ANTHROPIC_API_KEY, anthropic_model
...
from chatbot_graph_claude.chatbot_handler import ChatBot_Claude
from chatbot_graph_claude.prompt import system_prompt_claude
...
chatbot = ChatBot_Claude(ANTHROPIC_API_KEY, anthropic_model, system_prompt_claude) #system_prompt)#

# Function to load chat history
def load_chat_history():
    try:
        with open('backend/data_chat_history/chat_history_claude.json', 'r') as f:
            return json.load(f)
...
def save_chat_history(chat_history):
    with open('backend/data_chat_history/chat_history_claude.json', 'w', encoding='utf-8') as f:
...
*******************************************************************************
""")
    await message.reply(assistant_response["content"]) #, parse_mode="Markdown")
